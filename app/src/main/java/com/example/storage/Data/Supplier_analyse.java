...
public class Supplier_analyse extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View view = LayoutInflater.from(this).inflate(R.layout.activity_supplier_analyse,null);

        DrawableColor.changeColor(view,R.id.supplier_list,R.drawable.ic_list_white_24dp,R.color.checkin);
        DrawableColor.changeColor(view,R.id.point_supplier,R.drawable.ic_uncheck_box_black_24dp,R.color.checkin);
        DrawableColor.changeColor(view,R.id.standard,R.drawable.ic_check_box_black_24dp,R.color.checkin);
        DrawableColor.changeColor(view,R.id.day,R.drawable.ic_radio_button_checked_black_24dp,R.color.checkin);
        DrawableColor.changeColor(view,R.id.month,R.drawable.ic_radio_button_unchecked_black_24dp,R.color.checkin);

        setContentView(R.layout.activity_supplier_analyse);
    ...
    public void getStartDate(View v) {
        new DatePickerDialog(this, new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePicker view, int year, int monthOfYear,
                                  int dayOfMonth) {
                Supplier_analyse.this.year = year;
                month = monthOfYear;
                day = dayOfMonth;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.month:
                ...
                break;
            case R.id.activity:
                if (TextUtils.isEmpty(supplier_SKU.getText()) || TextUtils.isEmpty(supplier_name.getText()) || TextUtils.isEmpty(point_SKU.getText()) || TextUtils.isEmpty(point_name.getText())) {
                    Toast.makeText(this,"货品或供货商不能为空",Toast.LENGTH_SHORT).show();
                } else {
                    Intent intent = new Intent(this, Supplier_activity.class);
                    intent.putExtra("supplier_SKU",supplier_SKU.getText().toString());
                    intent.putExtra("supplier_name",supplier_name.getText().toString());
                    intent.putExtra("point_SKU",point_SKU.getText().toString());
                    intent.putExtra("point_name",point_name.getText().toString());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (resultCode) {
            case 1:
                point_SKU.setText(data.getExtras().getString("SKU"));
                point_name.setText(data.getExtras().getString("name"));
                break;
            case 2:
